var getRate = require('./rate');

function getCurrency(code, address, req, callback) {
  getRate(code, function(data) {
    if (data.rate) {
      var coinsValue = +(req.fiatValue / data.rate).toFixed(5);
...
module.exports = function(curCodes, address, req, callback) {
    var currs = [];
    curCodes.forEach(function(code) {
      getCurrency(code, address, req, function(curr) {
        currs.push(curr);
      })
    });
    var i = setInterval(function() {
      if(currs.length === curCodes.length) {
        clearInterval(i);
        callback(currs.filter(function(c) {
          return c !== null;
        }));
      }
    }, 100);
