def get_currencies(cur_codes, address, req)
  currs = []
  cur_codes.each do |code|
    cur = get_currency(code, address, req)
    if cur
      currs.push(cur)
    end
  end
...
def get_currency(code, address, req)
  rate = get_rate(code)

  if rate
    coins_value = (req[:fiatValue] / rate).round(5)
      ...
      when 'eth'
        {
            ...
            walvarLink: "ethereum:#{address}?amount=#{coins_value}&message=CHANGEME%2F%2F#{req[:invoice]}"
        }
      when 'etc'
        {
            address: address,
