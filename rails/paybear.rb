...
def get_address(order_id, token = 'ETH')
  payout_address = get_payout(token)
  callback_url = "http://CHANGEME.com/callback/#{order_id}"

  url = "https://api.paybear.io/v1/#{token.downcase}/payment/#{payout_address}/#{CGI.escape(callback_url)}"
...
def get_payout(token)
  ...
  if wallets[token.downcase.to_sym]
    wallets[token.downcase.to_sym]
  end
end
...
def get_currency(token, order_id, get_address = false)
  ...
  if rate && get_payout(token)
    fiat_value = 19.99 # get from order_id
    coins_value = (fiat_value / rate).round(5)
    currency = case token.downcase
                 when 'eth'
                   {
                     coinsValue: coins_value,
                     rate: rate,
                     title: 'Ethereum',
                     code: 'ETH',
                     metamask: true,
                 ...
                 when 'bch'
                   {
                     ...
                     blockExplorer: 'https://blockdozer.com/insight/address/',
                   }
                 when 'btg'
                   {
                     coinsValue: coins_value,
    ...
    if get_address
      ...
      currency[:blockExplorer] = currency[:blockExplorer] + currency[:address]
    else
      currency[:currencyUrl] = "/currencies?order=#{order_id}&token=#{token}"
    end
